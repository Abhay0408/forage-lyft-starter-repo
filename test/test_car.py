...
class Test_Calliope(unittest.TestCase):
    def test_needs_servicing(self):
        car = CarFactory.create_calliope(
            datetime.today().date(), datetime.today().date(), 35000, 0)
        self.assertTrue(car.needs_service())

    def test_does_not_need_servicing(self):
        car = CarFactory.create_calliope(
            datetime.today().date(), datetime.today().date(), 300, 0)
...
class Test_Glissade(unittest.TestCase):
    def test_needs_servicing(self):
        car = CarFactory.create_glissade(
            datetime.today().date(), datetime.today().date(), 65000, 0)
        self.assertTrue(car.needs_service())
...
class TestPalindrome(unittest.TestCase):
    def test_needs_servicing(self):
        car = CarFactory.create_palindrome(
            datetime.today().date(), datetime.today().date(), True)
        self.assertTrue(car.needs_service())

    def test_does_not_need_servicing(self):
        car = CarFactory.create_palindrome(
            datetime.today().date(), datetime.today().date(), False)
        self.assertFalse(car.needs_service())
